...
type Row = {
  ...
  permalink_url: string | null; tier: string | null; activity: string | null;
  track_count: number | null; followers_count: number | null;
  country_code: string | null; instagram: string | null;
};

const ACTIVITY_LABEL: Record<string, string> = {
  hot: "Активний", warming: "Пригасає", cooling: "Давно тихо", dormant: "Сплячий",
};

export function SegmentPreview({ previewUrl, downloadUrl, count }: { previewUrl: string; downloadUrl: string; count: number }) {
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={() => setOpen(false)}>
          <div className="flex max-h-[85vh] w-full max-w-5xl flex-col rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] shadow-2xl" onClick={(e) => e.stopPropagation()}>
            {/* Header */}
            <div className="flex items-center justify-between border-b border-[var(--border)] px-5 py-4">
              ...
              <div className="flex items-center gap-2">
                <a href={downloadUrl} download
                  className="flex items-center gap-2 rounded-xl bg-[var(--accent)] px-4 py-2 text-xs font-semibold text-white transition-colors hover:bg-[var(--accent-hover)]">
                ...
                <table className="w-full text-left text-xs">
                  <thead className="sticky top-0 bg-[var(--bg-card)] text-[10px] uppercase tracking-wider text-[var(--text-muted)]">
                    <tr className="border-b border-[var(--border)]">
                      <th className="px-4 py-2.5 font-medium">Артист</th>
                      <th className="px-4 py-2.5 font-medium">Email</th>
                      <th className="px-4 py-2.5 text-right font-medium">Треки</th>
                      <th className="px-4 py-2.5 text-right font-medium">Фоловери</th>
                      <th className="px-4 py-2.5 font-medium">Активність</th>
                      <th className="px-4 py-2.5 font-medium">Країна</th>
                    </tr>
                  </thead>
                  <tbody>
                    {rows.map((r, i) => (
                      <tr key={i} className="border-b border-[var(--border)]/50 hover:bg-white/[0.02]">
                        <td className="px-4 py-2.5">
                          <a href={r.permalink_url ?? "#"} target="_blank" rel="noreferrer" className="font-medium text-[var(--text)] hover:text-[var(--accent)]">
                            {r.full_name || r.username || "—"}
                          </a>
                        </td>
                        <td className="px-4 py-2.5 text-[var(--text-muted)]">{r.email ?? "—"}</td>
                        <td className="px-4 py-2.5 text-right tabular-nums">{r.track_count ?? 0}</td>
                        <td className="px-4 py-2.5 text-right tabular-nums">{(r.followers_count ?? 0).toLocaleString("uk-UA")}</td>
                        <td className="px-4 py-2.5 text-[var(--text-muted)]">{r.activity ? (ACTIVITY_LABEL[r.activity] ?? r.activity) : "—"}</td>
                        <td className="px-4 py-2.5 uppercase text-[var(--text-muted)]">{r.country_code ?? "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
