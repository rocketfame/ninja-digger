...
async function getData(sp: SP, source: ScSourceKey) {
  const SRC = SC_SOURCE[source].sql;
  ...
  const [stats, seg, reexDays, seed, preview, crawl] = await Promise.all([
    ...
    source === "graph"
      ? retryRow<{ frontier: number; expanded: number; today: number }>(`SELECT
          ...
          (SELECT COUNT(*) FROM sc_artists WHERE ${SRC} AND email_found_at::date = CURRENT_DATE)::int today`)
      : Promise.resolve(null),
  ]);
  return {
    total: stats?.total ?? 0, promoters: stats?.promoters ?? 0, gold: stats?.gold ?? 0, diamond: stats?.diamond ?? 0,
    segCount: seg?.c ?? 0, segEmail: seg?.e ?? 0, reexDays, seed, preview, crawl,
...
export default async function ScLeadsPage({ params, searchParams }: { params: Promise<{ source: string }>; searchParams: Promise<SP> }) {
  const source = scSource((await params).source);
  const sp = await searchParams;
  const d = await getData(sp, source);
  const qs = (extra: Partial<SP>) => {
    const m = { ...sp, ...extra };
    const parts = Object.entries(m).filter(([, v]) => v).map(([k, v]) => `${k}=${v}`);
    return `/sc-leads/${source}${parts.length ? `?${parts.join("&")}` : ""}`;
  };
  // Promoter toggle = analytics export (repost channels), otherwise real leads.
  const exportUrl = `/api/segments/soundcloud/export?source=${source}${qs({}).replace(`/sc-leads/${source}`, "").replace("?", "&").replace("promoter=1", "analytics=1")}`;
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-page)] text-[var(--text)]">
      ...
      <main className="mx-auto max-w-5xl px-5 py-10">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-2xl font-bold tracking-tight">{SC_SOURCE[source].label}</h1>
          <p className="mt-1 text-sm text-[var(--text-muted)]">
            {d.total.toLocaleString("uk-UA")} артистів у сегменті{source === "reex" && d.seed ? ` · джерело @${d.seed.permalink} (${d.seed.followers_count?.toLocaleString("uk-UA")} фоловерів)` : ""}
          </p>
          {/* Two engines, two segments — never blended, so each one's output stays visible. */}
          <div className="mt-4 flex gap-2">
            {(Object.keys(SC_SOURCE) as ScSourceKey[]).map((k) => (
              <Link key={k} href={`/sc-leads/${k}`}
                className={`rounded-lg border px-3 py-1.5 text-sm transition-colors ${k === source ? "border-transparent bg-[var(--accent)] text-black" : "border-[var(--border)] text-[var(--text-muted)] hover:text-[var(--text)]"}`}>
                {SC_SOURCE[k].label}
              </Link>
            ))}
          </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
          ...
          <div className="space-y-6">
            ...
            {source === "reex" ? (
              ...
              <section className="rounded-xl border border-[var(--border)] p-4">
                <h2 className="text-sm font-semibold">Обхід графа підписок</h2>
                <p className="mt-1 text-[11px] text-[var(--text-muted)]">Без списку сідів: кожен знайдений продюсер відкриває ~270 своїх підписок, 90% з них теж продюсери. Крон кожні 9 хвилин.</p>
                <dl className="mt-3 grid grid-cols-3 gap-3 text-center">
                  <div><dt className="text-[11px] text-[var(--text-muted)]">фронтир</dt><dd className="text-lg font-semibold">{(d.crawl?.frontier ?? 0).toLocaleString("uk-UA")}</dd></div>
                  <div><dt className="text-[11px] text-[var(--text-muted)]">розгорнуто</dt><dd className="text-lg font-semibold">{(d.crawl?.expanded ?? 0).toLocaleString("uk-UA")}</dd></div>
                  <div><dt className="text-[11px] text-[var(--text-muted)]">email сьогодні</dt><dd className="text-lg font-semibold">{(d.crawl?.today ?? 0).toLocaleString("uk-UA")}</dd></div>
                </dl>
              </section>
            )}
          </div>
          ...
          <aside className="lg:sticky lg:top-6 lg:self-start">
            <SendReadyCard
              ...
              footer={hasFilter ? <Link href={`/sc-leads/${source}`} className="mt-3 inline-block text-xs text-[var(--text-muted)] underline hover:text-[var(--text)]">скинути фільтри</Link> : undefined}
            />
          </aside>
        </div>
      </main>
