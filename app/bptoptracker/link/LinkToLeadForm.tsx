...
export function LinkToLeadForm({
  ...
  return (
    <div className="space-y-4 rounded-lg border border-[var(--border)] bg-[var(--bg-card)] p-4">
      <div>
        <label className="block text-sm font-medium text-[var(--text)] mb-1">Пошук ліда (імʼя або Beatport ID)</label>
        <input
          type="text"
          value={search}
      ...
      <div>
        <label className="block text-sm font-medium text-[var(--text)] mb-1">Обрати лід</label>
        <select
          value={selectedId}
          onChange={(e) => setSelectedId(e.target.value)}
          className="w-full rounded border border-[var(--border)] bg-[var(--bg-hover)] px-3 py-2 text-sm text-[var(--text)]"
        >
