...
const COLS = [
  ...
  "parent_label", "sublabels", "kind", "chart_entries", "chart_tracks", "best_position", "last_charted", "discovered_via", "updated_at",
] as const;

export async function GET(request: Request) {
  if (!isAuthorized(request)) return unauthorized();
  const sp = new URL(request.url).searchParams;
  const f = Object.fromEntries(["genre", "grade", "tier", "demo", "via", "q", "status", "kind"].map((k) => [k, sp.get(k) || undefined])) as LabelFilters;
  ...
  const { rows } = await pool.query(
    `SELECT l.name, l.grade, ARRAY_TO_STRING(l.genre_groups, '; ') genre_groups, ARRAY_TO_STRING(l.genres, '; ') genres,
            ...
            l.parent_label, ARRAY_TO_STRING(l.sublabels, '; ') sublabels, l.kind, l.chart_entries, l.chart_tracks, l.best_position, l.last_charted::text, l.discovered_via, l.updated_at::date::text
