...
async function getAnalytics() {
  const [
    ...
    pool.query<DayStat>("SELECT sent_at::date::text as date, COUNT(*)::int as count FROM outreach_events WHERE sent_at >= CURRENT_DATE - 14 GROUP BY date ORDER BY date DESC"),
    pool.query("SELECT COUNT(*)::int as c FROM outreach_events"),
    pool.query("SELECT COUNT(*)::int as c FROM outreach_events WHERE sent_at >= CURRENT_DATE"),
    pool.query("SELECT COUNT(*)::int as c FROM outreach_events WHERE sent_at >= CURRENT_DATE - 1 AND sent_at < CURRENT_DATE"),
    pool.query("SELECT COUNT(*)::int as c FROM outreach_events WHERE sent_at >= CURRENT_DATE - 7"),
    pool.query<StatusCount>("SELECT COALESCE(status, 'New') as status, COUNT(*)::int as count FROM lead_profiles GROUP BY status ORDER BY count DESC"),
    ...
    pool.query("SELECT COUNT(*)::int as c FROM lead_scores"),
    pool.query("SELECT COUNT(DISTINCT artist_beatport_id)::int as c FROM artist_contacts WHERE type='email' AND confidence >= 0.65 AND (status IS NULL OR status != 'bounced')"),
    ...
    pool.query("SELECT COUNT(*)::int as c FROM artist_contacts WHERE status = 'bounced'"),
