...
export function ArtistLeadCard({
  ...
  // --- Email variant pools: 5 subject + 5 body per touch to avoid spam filters ---
  const touch1Subjects = [
  ...
  // Pick a consistent random variant per artist (seeded by beatport ID)
  const variantSeed = useMemo(() => {
    let hash = 0;
    const id = String(artist.artist_beatport_id);
  ...
  const emailTemplates = useMemo(() => [
    {
      ...
      subject: touch1Subjects[variantSeed % touch1Subjects.length],
      body: touch1Bodies[variantSeed % touch1Bodies.length],
    ...
    },
    {
      ...
      subject: touch2Subjects[(variantSeed + 1) % touch2Subjects.length],
      body: touch2Bodies[(variantSeed + 1) % touch2Bodies.length],
    ...
    },
    {
      ...
      subject: touch3Subjects[(variantSeed + 2) % touch3Subjects.length],
      body: touch3Bodies[(variantSeed + 2) % touch3Bodies.length],
      color: "#f97316", bg: "rgba(249,115,22,0.08)", borderColor: "rgba(249,115,22,0.25)",
    },
  ], [displayName, variantSeed]);
