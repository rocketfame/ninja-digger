  ...
  if (type === "dead") {
    const res = await pool.query<SegmentRow>(
      `SELECT ac.artist_beatport_id, am.artist_name, LOWER(TRIM(ac.value)) AS email,
              ...
              ls.segment AS chart_segment, am.first_seen::text, am.last_seen::text
...
export type SegmentStats = { type: EmailSegmentType; label: string; count: number; lastUpdated: string | null };
...
export async function getSegmentStats(): Promise<SegmentStats[]> {
  ...
  const [noReply, warm, dead] = await Promise.all([
    q(
  ...
  return [
    { type: "no_reply", label: SEGMENT_LABELS.no_reply, ...noReply },
