-- Сегмент «Новачки»: артисти, які тільки залетіли в топ-чарт і ніколи до цього в ньому не були (total_days_in_charts = 1).

-- 1) Дозволити значення NEWCOMER у lead_scores
ALTER TABLE lead_scores DROP CONSTRAINT IF EXISTS lead_scores_segment_check;
ALTER TABLE lead_scores ADD CONSTRAINT lead_scores_segment_check CHECK (
  segment IN ('NEW_ENTRY', 'CONSISTENT', 'FAST_GROWING', 'DECLINING', 'TOP_PERFORMER', 'NEWCOMER')
);

-- 2) Логіка: NEWCOMER = лише 1 день у чарті (вперше з’явились), далі як раніше
