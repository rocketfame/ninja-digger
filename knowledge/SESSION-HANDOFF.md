...
**Left:** once eSputnik shows CONFIGURED → add sender "Max from Promosound <max@psg-offers.com>" (Settings → Відправники; refused while IN_PROGRESS), clone message 4681093 with the new sender + Reply-To → set `esputnik_message_soundcloud` to the new id. Then 48 h pause from DNS (earliest first send 19.09 16:00 Kyiv): `esputnik_daily_push=100`, `esputnik_batch_per_hour=25`, `esputnik_sc_source=reex`, and the ramp with gates from MASS-OUTREACH.md. Also open: who sends unaligned From @promosound.net (Postmaster "From: header alignment — Needs work"); Brevo stays paused.
