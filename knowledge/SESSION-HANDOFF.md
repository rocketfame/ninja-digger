...
## 26.09 — лагодження харвесту і черги лейблів (коміт 8f44e5e)

- **Re-Ex-сідовий харвест стояв з 10.09.** Фільтр «60 днів відпочинку» (коміт 142aa8b) діяв і на НЕДОЇДЕНІ сіди → 379 відкритих сідів (віддача 12 % email) випали з ротації; працював лише граф-обхід (1–5 %). Нових SC-email: 18.4k/день (15.09) → 5.7k (25.09). Фікс: відпочинок лише для доїдених. Watchdog тепер рахує сідовий харвест окремо від `graph:`.
- **Лейбли:** резолв тепер по колу між джерелами (Discogs-сублейбли 3.2k і блеклист 4.1k не резолвились жодного разу за 17.5k SC-кандидатами). Локальний воркер `com.ninjadigger.labels-local` падав на синтаксичній помилці — виправлено.
- Перевірка за день: сідовий приплив і SC-email/день мають піти вгору; черга лейблів — `new` по всіх `discovered_via` має зменшуватись.
